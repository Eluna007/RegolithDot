...
func powerServicePath() string {
	return "/etc/systemd/system/apollo-power.service"
}
...
func oldUserPowerServicePath() string {
	...
	return filepath.Join(home, ".config", "systemd", "user", "apollo-power.service")
}
...
func renderPowerService(gov string) string {
	script := fmt.Sprintf(
		...
		gov, gov)
	return "[Unit]\n" +
		"Description=Apollo Shell power profile\n\n" +
		"[Service]\n" +
		"Type=oneshot\n" +
...
func cleanupOldUserPowerService() {
	if old := oldUserPowerServicePath(); fileExists(old) {
		exec.Command("systemctl", "--user", "disable", "--now", "apollo-power.service").Run()
		os.Remove(old)
	}
}
...
func installPowerService(password, gov string) error {
	...
	if err := runSudo(password, "systemctl", "enable", "--now", "apollo-power.service"); err != nil {
...
func removePowerService(password string) error {
	cleanupOldUserPowerService()
	if err := runSudo(password, "systemctl", "disable", "--now", "apollo-power.service"); err != nil {
