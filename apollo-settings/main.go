...
func main() {
	// Headless: `apollo-settings apply` re-renders generated.lua from the
	...
	a.SetIcon(fyne.NewStaticResource("apollo-settings.png", iconBytes))
	a.Settings().SetTheme(apolloTheme{})
	w := a.NewWindow("Apollo Settings")
	w.SetIcon(fyne.NewStaticResource("apollo-settings.png", iconBytes))
...
func wizardScreen(cfg *Config, w fyne.Window) fyne.CanvasObject {
	...
	getStarted := widget.NewButton("Get Started", func() {
		...
		// saveConfig above creates config.json, which is what isFirstRun()
		// keys on — so the wizard won't reappear on next launch.
		w.SetContent(container.NewBorder(header(), nil, nil, nil, makeTabs(cfg, w)))
	})
	getStarted.Importance = widget.SuccessImportance

	welcome := widget.NewLabelWithStyle("Welcome to Apollo Shell", fyne.TextAlignCenter, fyne.TextStyle{Bold: true})
...
// ── Signature header: mauve "Apollo" wordmark + sun ─────────────────────
func header() fyne.CanvasObject {
	sun := canvas.NewText("☀️", nil)
	sun.TextSize = 24

	word := canvas.NewText("Apollo", hexToColor(cMauve))
...
func aboutTab(cfg *Config, w fyne.Window) fyne.CanvasObject {
	export := widget.NewButton("Export…", func() {
		...
		d.SetFileName("apollo-config.json")
		d.Show()
	})
	imp := widget.NewButton("Import…", func() {
		dialog.NewFileOpen(func(rc fyne.URIReadCloser, err error) {
			...
			*cfg = nc
			...
			_ = applyHypr(*cfg)
			_ = applyRofi(*cfg)
			dialog.ShowInformation("Imported", "Config imported and applied. Reopen Apollo Settings to refresh the controls.", w)
		}, w).Show()
	})
	...
	cfgCard := widget.NewCard("Config", "Back up, move, or reset your settings", container.NewVBox(
		...
		hintText("Exports ~/.config/apollo/config.json. Import replaces it and applies."),
		hintText("Auto-backup: every Apply snapshots config.json to ~/.config/apollo/backups/ (keeps last 50)."),
		resetAll,
	))

	health := widget.NewButton("Run health check", func() {
		home, _ := os.UserHomeDir()
		doc := filepath.Join(home, "RegolithDot", "scripts", "apollo-doctor")
		if _, err := os.Stat(doc); os.IsNotExist(err) {
			exe, _ := os.Executable()
			doc = filepath.Join(filepath.Dir(exe), "..", "..", "scripts", "apollo-doctor")
		}
	...
	healthCard := widget.NewCard("Diagnostics", "Check that everything is wired up correctly", container.NewVBox(
		health,
		hintText("Opens a terminal with a pass/fail summary. If anything's wrong, details are also saved to ~/.config/apollo/doctor.log."),
	))
	...
	screenshot := widget.NewButton("Export as PNG", func() {
		...
		d.SetFileName("apollo-config-card.png")
		d.Show()
	})
	...
	aboutCard := widget.NewCard("About", "", container.NewVBox(
		...
		repo,
		hintText("A remake of Moonlit-shell by Fi3w0, rebuilt on Hyprland's Lua config."),
		hintText("Made with ☀️"),
	))
...
func renderCard(cfg *Config) image.Image {
	...
	cardText(img, 38, 32, "APOLLO SHELL", cardPalette["mauve"], 2)
